...
import './App.css';
// Add Avatar and Group to your Mantine imports
...
const WS_URL = `ws://localhost:8000/ws/board/1/?username=${username}`;
function App() {
  ...
  // Load card data into the form when a card is selected
  useEffect(() => {
    if (selectedCard) {
      form.setValues({
 ...
 useEffect(() => {
    if (lastJsonMessage) {
      if (lastJsonMessage.type === 'board_update') {
        fetchBoard(); // Refetch board data when a message is received
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      axios.patch(`${API_URL}/cards/${draggableId}/`, { column: destColumn.id })
        .catch(err => {
          ...
          fetchBoard(); // If the API call fails, revert to the server's state
        });
    }
  };
